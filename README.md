...
<head>
<meta charset="UTF-8">
<title>Smart Vehicle Detection – ML + OCR + Law & Parking</title>
...
<div id="detectPage" class="page">
  ...
  <br>
  <button onclick="showPage('modulePage')">⬅ Back</button>
</div>
...
function analyzeImage(){
  ...
  document.getElementById("rulesBox").innerHTML = `
    <p>✔ Max Speed: 60 km/h</p>
    <p>✔ Seat Belt Mandatory</p>
  `;
  ...
  document.getElementById("violationBox").innerHTML =
    violations.map(v => <p>⚠ ${v}</p>).join("") +
    (fine ? <p>💰 Fine ₹${fine}</p> : "");

  document.getElementById("lawBox").innerHTML =
    violations[0] !== "No Violation"
      ? violations.map(v => <p>🚨 ${v} - ${vehicleData.plate}</p>).join("")
